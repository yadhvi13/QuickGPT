...
const [search, setSearch] = useState('')
  ...
  return (
    <div className={`flex flex-col h-screen min-w-[18rem] p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      {/* logo */}
      <img
       src={theme === 'dark' ? assets.logo_full : assets.logo_full_dark} alt="" className='w-full max-w-48' />
   ...
   <div className='flex items-center gap-2 p-3 mt-4 border border-gray-400 dark:border-white/20 rounded-md'>
      ...
      {/* recent chats*/}
      {chats.length > 0 && <p className='pt-3'>Recent chats</p>}
              ...
              <div onClick={()=> {navigate('/'); setSelectedChat(chat); setIsMenuOpen(false)}} key={chat._id} className='p-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group'>
                   <div>
                    ...
                    <p className='text-xs text-gray-500 dark:text-[#B1A6C0]'>
                      {/* we have used here moment package for real time */}
      ...
      <div
       ...
       className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
        ...
        <div className='flex flex-col text-sm'>
          ...
          <p className='text-xs text-gray-400 select-none'>Purchase Credits to use quickGPT</p>
        </div>
      </div>
      ...
      <div
       className='flex items-center justify-between gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md'>
        ...
        <label className='relative inline-flex cursor-pointer'>
          ...
          checked={theme === 'dark'} />
          <div className='w-9 h-5 bg-gray-400 rounded-full peer-checked:bg-purple-600 transition-all select-none'></div>
          <span className='absolute left-1 top-1 w-3 h-3 bg-white rounded-full transition-transform peer-checked:translate-x-4 select-none'></span>
        </label>
      </div>
...
{/*  */}
      ...
      onClick={()=> setIsMenuOpen(false)}
       src={assets.close_icon} className='absolute top-3 right-3 w-5 h-5 cursor-pointer md:hidden not-dark:invert' alt="" />
      </div>
  )
}
